...
void vga_set_character(uint8_t row, uint8_t col, char character, uint8_t attributes) {
  uint32_t offset = VIDEO_MEMORY_CTEXT + (row * Mode.width + col) * 2;
  isa_write_byte(offset, character);
  isa_write_byte(offset + 1, attributes);
}

void vga_set_blinking(bool enabled) {
    // Enter VGA attribute controller mode
    isa_inb(0x3DA); // Reset flip-flop by reading from input status register

    // Select Attribute Mode Control Register (Index 10h)
    isa_outb(0x3C0, 0x30);

    // Read current value of the Attribute Mode Control Register
    uint8_t modeControl = isa_inb(0x3C1);

    if (enabled) {
        // Set bit 3 to enable blinking
        modeControl |= 0x08;
    } else {
        // Clear bit 3 to disable blinking (enable high-intensity background)
        modeControl &= ~0x08;
    }

    // Write back the modified value to the Attribute Mode Control Register
    isa_outb(0x3C0, modeControl);

    // Exit VGA attribute controller mode
    isa_inb(0x3DA); // Reset flip-flop again (optional)
}
...
void vga_screen_clear(uint8_t color) {
  ...
  } else if (Mode.mode == MODE12H) {
    vga_mode12h_screen_clear(color);
  } else if (Mode.mode == MODE03H) {
    uint32_t addr = VIDEO_MEMORY_CTEXT;
    for (uint16_t i = 0; i < Mode.width_bytes; i++) {
      isa_write_word(addr, (((uint16_t)color) << 8) | ' ');
      addr += 2;
    }
  }
}
